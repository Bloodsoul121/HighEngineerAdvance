package com.blood.highengineeradvance.toutiao.recyclerview;
...
public abstract class BaseRecyAdapter<T> extends RecyclerView.Adapter<BaseRecyViewHolder> {
    ...
    private void bindCommonItem(final BaseRecyViewHolder holder, final int position) {
        final T t = getItemData(holder, position);
    ...
    protected T getItemData(BaseRecyViewHolder holder, int position) {
    ...
    public interface OnRecyclerViewItemClickListener<T> {
        void onItemClick(BaseRecyViewHolder holder, T data, int position);
    }
    ...
    public void addData(final List<T> datas, final int position) {
        if (position > mDatas.size() || position < 0) {
            return;
        }
...
//                notifyDataSetChanged();
    ...
    public void addData(List<T> datas) {
        addData(datas, mDatas.size());
    }

    public void addData(final T data, final int position) {
        if (position > mDatas.size() || position < 0) {
            return;
        }
        mRecyclerView.post(new Runnable() {
